...
def objective_function(params):
    ...
    expectation_value = estimator.run(bound_circuit, hamiltonian).result().values[0]
    return expectation_value.real

initial_point = np.random.rand(ansatz.num_parameters) * 2 * np.pi
result = minimize(objective_function, initial_point, method='COBYLA', options={'maxiter': 200})
...
for i in range(coupled_data.shape[1]):
    plt.plot(time, coupled_data[:, i], '.', label=f'Data Channel {i+1}')
    plt.plot(time, mean_prediction_advanced[i::channels], '-', label=f'Advanced Mean Prediction Channel {i+1}')
    plt.fill_between(time, 
                     mean_prediction_advanced[i::channels] - 2*std_prediction_advanced[i::channels], 
                     mean_prediction_advanced[i::channels] + 2*std_prediction_advanced[i::channels], 
                     alpha=0.3)
plt.xlabel('Time')
plt.ylabel('Correlation')
